<div class="vc_row wpb_row vc_row-fluid padding-top-5vw padding-bottom-5vw vc_column-gap-10 vc_row-o-equal-height vc_row-o-content-middle vc_row-flex">
    <div class="wpb_column vc_column_container vc_col-sm-6 vc_col-lg-6 vc_col-md-6 vc_col-xs-12">
        <div class="vc_column-inner">
            <div class="wpb_wrapper">
                <div class="wpb_text_column wpb_content_element  vc_custom_1595860675178 center-at-768">
                    <div class="wpb_wrapper">
                        <h1>Transform the way you engage with your constituents</h1>
                    </div>
                </div>

                <div class="wpb_text_column wpb_content_element ">
                    <div class="wpb_wrapper">
                        <div>We built the iConstituent Engagement Platform with the belief that secure, efficient, impactful communications can bring people and government closer together.</div>
                    </div>
                </div>

                <div class="vc_row wpb_row vc_inner vc_row-fluid vc_custom_1594345767353 vc_column-gap-20">
                    <div class="wpb_column vc_column_container vc_col-sm-6 vc_col-lg-6 vc_col-md-6 vc_hidden-md vc_hidden-sm vc_col-xs-6 vc_hidden-xs">
                        <div class="vc_column-inner">
                            <div class="wpb_wrapper">
                                <div class="vc_btn3-container  right-arrow vc_btn3-center">
                                    <a class="vc_general vc_btn3 vc_btn3-size-md vc_btn3-shape-rounded vc_btn3-style-outline vc_btn3-block vc_btn3-color-grey" href="#explorecep" title="">Explore Products</a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="wpb_column vc_column_container vc_col-sm-6 vc_col-lg-6 vc_col-md-6 vc_hidden-md vc_hidden-sm vc_col-xs-6 vc_hidden-xs">
                        <div class="vc_column-inner">
                            <div class="wpb_wrapper">
                                <div class="vc_btn3-container  ctaform vc_btn3-center" data-popup-id="54251">
                                    <button class="vc_general vc_btn3 vc_btn3-size-md vc_btn3-shape-rounded vc_btn3-style-flat vc_btn3-block vc_btn3-color-grey">Book a Demo</button>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="vc_row wpb_row vc_inner vc_row-fluid button-padding-5px max-width-400-at-768 vc_custom_1594562766987">
                    <div class="wpb_column vc_column_container vc_col-sm-6 vc_col-lg-6 vc_hidden-lg vc_col-md-6 vc_col-xs-6 vc_hidden-xs">
                        <div class="vc_column-inner">
                            <div class="wpb_wrapper">
                                <div class="vc_btn3-container  right-arrow vc_btn3-center">
                                    <a class="vc_general vc_btn3 vc_btn3-size-md vc_btn3-shape-rounded vc_btn3-style-outline vc_btn3-block vc_btn3-color-grey" href="#explorecep" title="">Explore Products</a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="wpb_column vc_column_container vc_col-sm-6 vc_col-lg-6 vc_hidden-lg vc_col-md-6 vc_col-xs-6 vc_hidden-xs">
                        <div class="vc_column-inner">
                            <div class="wpb_wrapper">
                                <div class="vc_btn3-container vc_btn3-center">
                                    <a class="vc_general vc_btn3 vc_btn3-size-md vc_btn3-shape-rounded vc_btn3-style-flat vc_btn3-block vc_btn3-color-grey" href="/schedule-a-demo/" title="">Book a Demo</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="wpb_column vc_column_container vc_col-sm-6 vc_col-lg-6 vc_col-md-6 vc_col-xs-12">
        <div class="vc_column-inner">
            <div class="wpb_wrapper">
                <div class="wpb_single_image wpb_content_element vc_align_center   max-width-400-at-768">
                    <figure class="wpb_wrapper vc_figure">
                        <div class="vc_single_image-wrapper   vc_box_border_grey">
                            <img width="420" height="332" src="https://iconstituent.com/wp-content/uploads/Group-17421.svg" class="vc_single_image-img attachment-full" alt="">
                        </div>
                    </figure>
                </div>

                <div class="vc_row wpb_row vc_inner vc_row-fluid max-width-400-at-768 vc_custom_1594566139961">
                    <div class="wpb_column vc_column_container vc_col-sm-6 vc_col-lg-6 vc_hidden-lg vc_col-md-6 vc_hidden-md vc_hidden-sm vc_col-xs-6">
                        <div class="vc_column-inner vc_custom_1594566219100">
                            <div class="wpb_wrapper">
                                <div class="vc_btn3-container  right-arrow vc_btn3-center">
                                    <a class="vc_general vc_btn3 vc_btn3-size-md vc_btn3-shape-rounded vc_btn3-style-outline vc_btn3-block vc_btn3-color-grey" href="#explorecep" title="">Explore Products</a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="wpb_column vc_column_container vc_col-sm-6 vc_col-lg-6 vc_hidden-lg vc_col-md-6 vc_hidden-md vc_hidden-sm vc_col-xs-6">
                        <div class="vc_column-inner vc_custom_1594566228569">
                            <div class="wpb_wrapper">
                                <div class="vc_btn3-container vc_btn3-center">
                                    <a class="vc_general vc_btn3 vc_btn3-size-md vc_btn3-shape-rounded vc_btn3-style-flat vc_btn3-block vc_btn3-color-grey" href="/schedule-a-demo/" title="">Book a Demo</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
